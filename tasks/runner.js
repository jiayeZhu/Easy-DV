...
rl.on('line', (input) => {
  if (input == 'rs') {
    exit(0);
    isElectronOpen = false;
    setTimeout(()=>{run(`webpack-dev-server --hot --colors --config webpack.renderer.config.js --port ${config.port} --content-base app/dist`, YELLOW, 'webpack')},500);
    // treeKill(children[1].pid,'SIGTERM', () => {console.log('done');children.pop()});
  }
  // else if (input == 'u') {
  //   console.log(children);
  //   run('electron app/src/main/index.dev.js', BLUE, 'electron');
  // }
});